  ...
  return (
    <Container>
      {perksData.map((perk, i) => (
        <div
          key={i}
          className={`col-span-12 lg:col-span-10 lg:col-start-2 grid grid-cols-1 lg:grid-cols-2 border-t border-p-el py-5 md:p-5 md:m-5 gap-10 xl:gap-28 lg:p-10 lg:m-10 items-center justify-between`}
        >
          <div
          ...
          <div
            className={`${
              i % 2 === 0 ? 'lg:col-start-2' : 'lg:col-start-1'
            } row-start-1`}
          >
            <Image
              src={perk.image}
              alt={perk.title}
              className='h-full w-max hover:cursor-pointer'
              width={700}
              height={700}
            />
          </div>
